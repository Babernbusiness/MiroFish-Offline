...
class ParallelIPCHandler:
    ...
    async def handle_interview(
        ...
        if success_count > 0:
            ...
            print(
                f"  Interview completed: agent_id={agent_id}, success_platforms={success_count}/{len(platforms_to_interview)}"
            )
            return True
        else:
            errors = [
...
async def run_twitter_simulation(
    ...
    for round_num in range(total_rounds):
        ...
        if _shutdown_event and _shutdown_event.is_set():
            if main_logger:
                main_logger.info(
                    f"Received exit signal，at round {round_num + 1} stop simulation"
                )
            break
    ...
    log_info(
        f"Simulation loop completed! Time taken: {elapsed:.1f}seconds, Total actions: {total_actions}"
    )
...
async def main():
    ...
    log_manager.info("=" * 60)
    log_manager.info(
        f"Simulation loop completed! Total time: {total_elapsed:.1f}seconds"
    )
